...
A process apart from the indexer loop. Disk events become
`indexar(..., prefixo=)` — the same call the CLI uses for a subtree. Pause and
cancel stay `comando.txt`; the bar stays `progresso.json`. No new IPC.
...
class Observador:
    ...
    def enfileirar(self, path: Path) -> None:
        if not self.candidato(path):
            return
        with self._fila:
            self._pendentes[str(path)] = time.monotonic()

    def drenar(self, agora: float | None = None) -> list[str]:
        """Index paths whose debounce has elapsed. Returns the `prefixo` list."""
        agora = time.monotonic() if agora is None else agora
        with self._fila:
            maduros = [
                Path(p)
                for p, quando in self._pendentes.items()
        ...
        feitos: list[str] = []
        for path in maduros:
            prefixo = self._indexar_um(path)
            if prefixo:
                feitos.append(prefixo)
        return feitos

    def _indexar_um(self, path: Path) -> str | None:
        prefixo = self.prefixo_de(path)
        if prefixo is None or not self.candidato(path):
            return None
