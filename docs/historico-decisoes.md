# Histórico de decisões — F0 a F4 (crônica)

> **Extraído do `CLAUDE.md` em 25/08/2026, sem alterar uma linha do conteúdo.**
> Motivo: o retomador tinha 688 linhas, e 79% eram esta crônica — trinta blocos
> de ablação do acervo corporativo lidos a cada sessão. O efeito não era neutro:
> quem abre a sessão lendo a crônica **continua a crônica**, e a régua do produto
> é [`regra-de-ouro.md`](regra-de-ouro.md) — base desconhecida, usuário leigo.
>
> Nada aqui foi apagado, e nada aqui é obsoleto: é a evidência datada de cada
> decisão, com corpus declarado. O que mudou é **quando** se lê. O `CLAUDE.md`
> guarda as lições que valem para qualquer acervo; os números que valem para
> *este* acervo ficam aqui, e se consultam quando a pergunta é sobre eles.
>
> Regra de leitura: todo número abaixo é do **acervo corporativo**, nesta
> máquina, na data que o bloco declara (`colaboracao.md` §4, regra 7). Nenhum
> deles autoriza mudar `[padrao]` por si só (regra 10).
>
> Única alteração de texto: o título da seção abaixo, que no `CLAUDE.md` era
> "Estado atual" — aqui ele não é mais o estado, é o histórico.

---

## Crônica — F0 a F4, na ordem em que aconteceu
...
Ver [`docs/painel-de-ajuste.md`](painel-de-ajuste.md) e a seção de bases do
...
E1–E6 e Q1–Q10.** Ler [`docs/relatorio-avaliacao-resiliente.md`](relatorio-avaliacao-resiliente.md),
[`docs/guia-engenharia-5-estrelas.md`](guia-engenharia-5-estrelas.md) e, antes
de tocar no gerador, o laudo [`docs/avaliacao-pacote-e1.md`](avaliacao-pacote-e1.md).
